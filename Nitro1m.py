import random, requests, json, string, threading
...
def generate(disc_token, proxy):
    while True:
        try:
            while True:
                email = ran(10) + random.choice(["@gmail.com", "@nicedayz.fun", "@yahoo.com", "@discordsupport.space", "@nicedayzemail.fun", "@nicedayzhole.fun", "@voicerecorder.fun"])
                ...
                print(F"[INFO]  nitroLink {nitroLink.text}")
...
for disc_token in open("Token Worked.txt","r").read().splitlines():
    proxy = random.choice(open("proxy.txt","r").read().splitlines()); proxyDict = {"http": f"http://{proxy}","https": f"http://{proxy}"}
